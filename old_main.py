...
FILEPATH = "data/voc.txt"
RESET = 0  # POZOR resetuje všechny body a přírustek bodů
"""win = Tk()
...
""")
    ...
    if RESET:  # POZOR RESET všech bodů na :0:0 !!!!!!!!!
        ...
        f.write("".join(lines))
        f.close()
        print("Byl proveden reset bodů a přírustek bodů.")
        sys.exit(5)
        ...
        for i in range(loop_num):
            ...
            for k, elem in enumerate(incorrect_choice):
                ...
                if image_A.startswith("[") and image_A.endswith("]"):
                    ...
                    image_index_A += [k]
            # open_image(str(i+1)+"/"+str(loop_num)+") ", image_Q,
            # image_index_A, image_list_A, f, lines)
